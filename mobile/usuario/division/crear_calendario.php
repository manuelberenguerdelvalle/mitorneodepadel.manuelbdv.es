		...
		if($idayvuelta == 'S'){//GRUPOS
			...
			for($i=1; $i<=$num_grupos; $i++){//grupos
				...
				if($cont_total > 0){//personalizados
					...
					if($a % 2 == 0){$num_partidos = intval($a / 2);}//obtenemos los partidos por jornada
					else{//si son impartes sumamos un partidos
						$impar = true;
						$num_partidos = intval(($a / 2)) + 1;
					}
					//SOLO 2 JORNADAS FIJAS CON X PARTIDOS				
					$num_equipos_temp=count($equipos_temp);//numero de equipos del grupo
					if($num_equipos_temp % 2 == 0){$mitad_equipos = intval($num_equipos_temp / 2);}//obtenemos la mitad para los pares
					else{$mitad_equipos = intval(($num_equipos_temp / 2)) + 1;}//obtenemos la mitad para los impares
			...
			$eliminatoria = intval($num_equipos / 2); //obtenemos la cantidad de eliminatorias
