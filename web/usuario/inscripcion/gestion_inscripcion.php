...
LAS DEVOLUCIONES ONLINE SON IMPOSIBLES, LOS JUGADORES NO VAN A TENER RETORNO AUTOMATICO NI CUENTA PREMIERE
			...
			$opcion = $_SESSION['opcion'];
			 ...
			 if($datos_rec[0] == 'SUCCESS' && obten_resDespuesIgual($datos_rec[15]) == $_GET["tx"]){//SI ES CORRECTO CONTINUO
			 	if(obten_consultaUnCampo('unicas_torneo','COUNT(id_pago_admin)','pago_admin','transaccion',$_GET["tx"],'','','','','','','') == 0){
...
/*
                     ...
                     $id_devolucion = obten_consultaUnCampo('unicas_torneo','id_pago_admin','pago_admin','liga',$inscripcion->getValor('liga'),'division',$inscripcion->getValor('division'),'equipo','-1','','','');
